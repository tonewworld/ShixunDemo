...
TSubclassOf<AActor> UInventoryComponent::GetActorClassForItem(EItemID ItemID)
{
    ...
    case EItemID::Key_1:
        return TSoftClassPtr<AActor>(FSoftObjectPath(TEXT("/Game/Assets/Blueprints/BP_Key_1.BP_Key_1_C"))).LoadSynchronous();
    case EItemID::Key_2:
        return TSoftClassPtr<AActor>(FSoftObjectPath(TEXT("/Game/Assets/Blueprints/BP_Key_2.BP_Key_2_C"))).LoadSynchronous();
    case EItemID::Key_3:
        return TSoftClassPtr<AActor>(FSoftObjectPath(TEXT("/Game/Assets/Blueprints/BP_Key_3.BP_Key_3_C"))).LoadSynchronous();
    case EItemID::Ball:
...
FText UInventoryComponent::GetItemDisplayName(EItemID ItemID)
{
    ...
    case EItemID::Key_1:        return NSLOCTEXT("Item", "Key_1", "钥匙1");
    case EItemID::Key_2:        return NSLOCTEXT("Item", "Key_2", "钥匙2");
    case EItemID::Key_3:        return NSLOCTEXT("Item", "Key_3", "钥匙3");
...
int32 UInventoryComponent::GetCollectedKeyCount() const
{
    int32 Count = 0;
    if (HasItem(EItemID::Key_1)) Count++;
    if (HasItem(EItemID::Key_2)) Count++;
    if (HasItem(EItemID::Key_3)) Count++;
    return Count;
}

bool UInventoryComponent::HasAllThreeKeys() const
{
    return GetCollectedKeyCount() == 3;
}

bool UInventoryComponent::OfferThreeKeys()
{
    if (!HasAllThreeKeys()) return false;

    TArray<int32> KeySlots = {
        FindItem(EItemID::Key_1),
        FindItem(EItemID::Key_2),
        FindItem(EItemID::Key_3)
    };
    KeySlots.Sort([](int32 A, int32 B) { return A > B; });
    for (int32 Slot : KeySlots)
    {
        RemoveItem(Slot);
    }

    OnThreeKeysOffered.Broadcast();
    return true;
}